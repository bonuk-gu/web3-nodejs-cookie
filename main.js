...
function authIsOwner(request, response){
    ...
    console.log(cookies);
    if(cookies.email === 'bonwook.koo@patterntech.co.kr' && cookies.password === '0816'){
        isOwner = true;
    }
    console.log(isOwner);
...
var app = http.createServer(function(request, response) {
    ...
    if(pathname === '/') {
        ...
        fs.readdir('./data', function(error, filelist){
            var title = 'Login';
            var list = template.list(filelist);
            var html = template.html(title, list, `<form action="/login_process" method="post">
        ...
        request.on('end', function(){
            var post = qs.parse(body);
            if(post.email === 'bonwook.koo@patterntech.co.kr' && post.password === '0816') {
